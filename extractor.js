...
function applyDuplicateTagsToHistory(history, tags, tagAddresses) {
  ...
  return history.map((entry) => {
    ...
    for (const [name, data] of Object.entries(entry.contracts)) {
      ...
      if (userTag) {
        // User provided a tag - always use it
        taggedContracts[`${baseName}#${userTag}`] = data;
      } else if (existingTag && !isAutoGeneratedTag(existingTag)) {
        // Entry has a human-readable tag - preserve it
        taggedContracts[`${baseName}#${existingTag}`] = data;
      } else if (needsTagging.has(baseName)) {
        // Conflict exists - use short hash or keep untagged
        if (hash) {
          taggedContracts[`${baseName}#${getShortHash(hash)}`] = data;
        } else {
